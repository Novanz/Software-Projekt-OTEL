...
def ask_ollama(prompt: str):
    with mlflow.start_span("ollama_inference") as span:
        ...
        span.set_inputs({"model": OLLAMA_MODEL, "prompt_preview": prompt[:500]})
        span.set_outputs({"answer_preview": answer[:500]})

        span.set_attribute("inference.latency_sec", round(latency, 3))
        span.set_attribute("ollama.prompt_eval_count", input_tokens)
        span.set_attribute("ollama.eval_count", output_tokens)
        span.set_attribute("ollama.total_tokens", total_tokens)
        span.set_attribute("ollama.prompt_eval_duration_ns", data.get("prompt_eval_duration", 0))
        span.set_attribute("ollama.eval_duration_ns", data.get("eval_duration", 0))

        # MLflow / OpenTelemetry-style GenAI attributes
