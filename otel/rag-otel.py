...
# MLflow docs say to disable MLflow's default tracer provider when combining
# OpenTelemetry and MLflow in the same application.
...
def setup_tracing():
    ...
    # MLflow docs say set_destination() must be called after configuring
    # your explicit TracerProvider.
        ...
        with mlflow.start_span("chroma_retrieval") as span:
            try:
                ...
                if otel_span is not None:
                    ...
                    otel_span.set_attribute("retrieval.hit_count", len(ranked))

                    if OTEL_CAPTURE_CONTENT:
                        otel_span.set_attribute("retrieval.query", query[:500])
...
def build_prompt(query: str, docs: list[dict]):
    ...
    with start_otel_span(
        ...
        kind=kind,
        attributes={"prompt.doc_count": int(len(docs))},
    ) as otel_span:
...
Answer:"""
                ...
                if otel_span is not None:
                    ...
                    otel_span.set_attribute("prompt.doc_ids", json.dumps([d["id"] for d in docs]))
                    otel_span.set_attribute(
                        "prompt.preview",
                        prompt[:500] if OTEL_CAPTURE_CONTENT else "[redacted]",
                    )
                    set_otel_ok(otel_span)
            ...
            try:
                ...
                if otel_span is not None:
                    ...
                    otel_span.add_event(
                        ...
                        {
                            "message_count": 2,
                            "prompt_preview": prompt[:300] if OTEL_CAPTURE_CONTENT else "[redacted]",
                        },
                    )
                    otel_span.add_event(
                        "gen_ai.response.summary",
                        {
                            "answer_preview": answer[:300] if OTEL_CAPTURE_CONTENT else "[redacted]",
    ...
    with start_otel_span("rag_request.otel", kind=kind, attributes=root_attrs) as otel_span:
        try:
            ...
            if otel_span is not None:
                otel_span.set_attribute(
                    "rag.query",
                    query[:300] if OTEL_CAPTURE_CONTENT else "[redacted]",
                )
