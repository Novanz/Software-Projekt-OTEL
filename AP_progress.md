...
| 4.4         | OTLP-Export aus MLflow       | Konfiguration des OTLP-Exports an den OTel-Collector, Prüfung Dual-Export **(TO BE DECIDED)** | ❌         | Nicht implementiert. Kein OTLP-Export an einen Collector, kein Dual-Export                             |
...
| 5.1.2       | OTLP-Exporter           | OTLP-gRPC/HTTP-Exporter, BatchSpanProcessor, Verbindung zum Collector               | ❌         | Kein `OTLPSpanExporter`/`BatchSpanProcessor`. Spans gelangen ausschließlich über `mlflow.tracing.set_destination` ins MLflow-Backend — keine echte OTLP-Pipeline.                      |
...
| 5.2.5       | Metrics                     | Token-Usage-, Latenz- und Fehler-Metriken gemäß GenAI-Metric-Konvention                | ❌         | **Keine Metrics** (kein Meter/Counter/Histogram). Latenz nur als Span-Attribut, keine Token-/Fehler-Metriken. Zentrale Lücke für Variante B.                                    |
